...
So far, we are going to depend on gagm and undum.  Undum is a framework for creating stories in javascript and html.  gagm is a common lisp framework for the creation of high level languages that can be later converted to a set of output languages.
...
This is done by executing the function =(initialize-undum)=, and define the filename for the html containing the story.  In this case, it will be =/tmp/clif.html=.
